...
>
> **LFS** objects are not in the clone either unless you explicitly fetch them — see [Big files, or how Git meets its limits](P4C4.md "Big files, or how Git meets its limits"), which is also where the "does my server support LFS?" question gets answered, because LFS needs cooperation from the far end and not every host provides it.
>
...
And if what you actually want is not a forge but automatic deployment when you push, that is a `post-receive` hook on precisely the kind of bare repository we just built — twelve lines of shell. We build one end to end in [Deploying a simple static site](P5C3.md "Deploying a simple static site").
