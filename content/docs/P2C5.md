...
At the end of [Collaborate with Git](P2C1.md "Collaborate with Git") we made a promise, and we should keep it. We said:
...
There is no technical reason. `git push origin master` works. Git has no notion of an important branch; `master` and `main` are pointers exactly like every other branch, as we saw in [Playing with our revisions](P1C7.md "Playing with our revisions").
...
* **The main branch is a promise.** In most teams it means "this is what we deploy", or at least "this is green". Anything that lands on it without passing the tests breaks that promise for everyone at once. See [A little structure please](P2C4.md "A little structure please") for the stable-master and unstable-master schools.
* **Bisectability.** A main branch made of reviewed, tested, self-contained commits can be bisected. We will use `git bisect` in [Keep a clean history, recover from mistakes](P2C6.md "Keep a clean history, recover from mistakes"), and you will want that property.
...
* **Required status checks** — the change cannot be integrated until the test suite has passed on it. This is where [Continuous integration with Git](P5C2.md "Continuous integration with Git") plugs in.
...
Back in [Inside the repository, inside the commit](P1C5.md "Inside the repository, inside the commit") we mentioned, almost in passing, that a commit "primarily identifies a particular **tree** and one (or for that matter several) parent **commit**s". Here is the several:
...
Both files are gone, in a single new commit, without any history being rewritten. There is a famous trap waiting on the other side of this operation; we defuse it in [the next chapter](P2C6.md "Keep a clean history, recover from mistakes").
...
`git log --graph` is the other half of the answer, and `git log --graph --oneline --decorate --all` is worth an alias. See [Making the command line yours](P3C1.md "Making the command line yours").
...
Use `--force-with-lease`, never bare `--force`. It refuses the push if the remote branch has moved since you last fetched, which is precisely the "oh no, somebody else *was* working here" case. See [Retrieve and send code](P2C3.md "Retrieve and send code").
...
* `git mergetool` — launch a three-pane graphical merge tool, configured with `merge.tool`. Meld, Beyond Compare, `vimdiff`, your IDE. Worth setting up once; see [Git With IDEs](P3C4.md "Git With IDEs").
...
[Playing with our revisions](P1C7.md "Playing with our revisions") introduced the basic form. Here is the version you actually need in a repository with history.
...
Everything in this chapter has been about doing things well. The next one is about the other half of the job: [Keep a clean history, recover from mistakes](P2C6.md "Keep a clean history, recover from mistakes").
