...
* `.gitattributes` is the sibling file — how Git *treats* files rather than whether it sees them: `* text=auto`, `eol=lf`, `*.png binary`, linguist hints, custom diff and merge drivers, and where **Git LFS** lives ([Git LFS](P4C4.md "Git LFS")). `git check-attr` shows the result.
* Ignoring a secret protects nothing. A committed, pushed secret is compromised forever: **rotate it**, then clean the history ([Keep a clean history, recover from mistakes](P2C6.md "Keep a clean history, recover from mistakes")).
