...
It is also simple politeness: integrating their work into yours, in your own repository, where a conflict is your problem to solve calmly, is nicer than making them integrate around you. More on the etiquette of this in [Implement an efficient collaborative workflow](P2C5.md "Implement an efficient collaborative workflow").
...
We have not properly introduced tags yet — that happens in [A little structure please](P2C4.md "A little structure please"), along with why annotated ones are the ones that matter. For now: remember that `git push` does not send them.
...
That is success. "Does not provide shell access" is not an error, it is GitHub being friendly. If instead you get `Permission denied (publickey)`, go to [Configure Git with an SSH key](P6C2.md "Configure Git with an SSH key") and start from the top.
