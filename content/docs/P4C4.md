...
Remember [Inside the repository, inside the commit](P1C5.md "Inside the repository, inside the commit"): a **commit** points to a **tree**, a **tree** lists **blob**s, and a **blob** is *the entire content of one version of one file*. Git does not store diffs. It stores snapshots.
...
We met `.gitattributes` in [A little structure please](P2C4.md "A little structure please"). Four attributes, each doing a job:
...
* **Xet** — content-defined chunking with deduplication *below* the file level, so changing a fraction of a large file uploads a fraction of the bytes. It is Hugging Face's storage backend now, and `git-xet` exists as an LFS custom transfer agent for talking to the Hub. It is a genuinely good idea and we cover it properly in [Git for data and models](P4C5.md "Git for data and models").
