...
Note the `>>`. Appending, not overwriting — `>` would delete every other key that account uses, including possibly your own from another machine. That single line in `authorized_keys` is the whole of what a self-hosted Git server needs from you; see [Git hosting](P3C2.md "Git hosting").
...
More about tokens — classic versus fine-grained, scopes, expiry — is in [Create and configure your GitHub or GitLab account](P6C3.md "Create and configure your GitHub or GitLab account").
...
`-s` signs; the backend it uses is whatever `gpg.format` says, so having set `ssh` above we get an SSH signature. What tags are and why annotated ones are better than lightweight ones is in [A little structure please](P2C4.md "A little structure please").
