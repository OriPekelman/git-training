...
It does, and it is called `git reflog`. We met it briefly in [Collaborate with Git](P2C1.md "Collaborate with Git"); here is what it is really for.
...
`--no-edit` keeps the existing message and skips your editor. As we saw in [the previous chapter](P2C5.md "Implement an efficient collaborative workflow"), `--amend` does not modify the commit — it builds a new one and moves the branch to it, so the **SHA** changes. The old commit is still in the reflog.
...
This is the third face of `git reset`, and now is a good moment to put all three side by side. [Playing with our revisions](P1C7.md "Playing with our revisions") showed you two of them:
...
* **`.gitignore` your secret files before you write them.** See [A little structure please](P2C4.md "A little structure please"). `.env`, `*.pem`, `credentials.json`, `id_rsa`. Commit a `.env.example` with the keys and no values.
...
If you have been marking your fixups as you go with `git commit --fixup <commit>` — see [the previous chapter](P2C5.md "Implement an efficient collaborative workflow") — then `--autosquash` builds most of the to-do list for you.
...
**What to do:** `git stash`. We met it in [Collaborate with Git](P2C1.md "Collaborate with Git"); here is the full set.
...
* **`git stash push -u -m '…'`**, then `stash list` / `show -p` / `apply` / `pop` / `drop` / `branch`. A stash is just a commit with two or three parents. For working on two things at once, a [worktree](P4C1.md "One repository, many working trees") is usually better.
