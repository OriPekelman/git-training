...
There it is. A per-worktree **HEAD**, in exactly the format we dissected in [Playing with our revisions](P1C7.md "Playing with our revisions") — `ref: refs/heads/<branch>` when attached, a bare **SHA** when detached. All that time we spent staring at `.git/HEAD` was for this moment.
...
Which is [Worktrees and agents, parallel work at machine speed](P4C2.md "Worktrees and agents, parallel work at machine speed").
